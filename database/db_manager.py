...
from config import DATABASE_NAME_3
...
def init_db_3():
    ...
    conn = None
    try:
        conn = sqlite3.connect(DATABASE_NAME_3)
        cursor = conn.cursor()
        ...
        conn.commit()
        print(f"Base de datos {DATABASE_NAME_3} y tabla creadas correctamente.")
        return conn  # Devuelve la conexión para su uso posterior
    except sqlite3.Error as e:
        print(f"Error al conectar o crear la base de datos: {e}")
    finally:
...
conn_3 = init_db_3() # Crear y conectar a la tercera base de datos
...
if conn_3:
    print(f"Conexión a {DATABASE_NAME_3} establecida.")
